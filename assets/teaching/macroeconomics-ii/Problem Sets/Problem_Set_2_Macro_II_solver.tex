...
El Lagrange asociado al problema del hogar es
\[
...
De la primera condición, $\lambda=1/C$. Al introducirla en la segunda condición se obtiene
\[
...
\[
  C=\frac{w\ell}{\psi}.
\]
Al sustituir esta expresión en la tercera condición, se obtiene
\[
  \frac{w\ell}{\psi}=w(1-\ell)+\pi-T.
\]
Multiplicando por $\psi$ y reuniendo los términos que contienen $\ell$,
\[
...
\]
Por lo tanto, la demanda de ocio es
\[
  \boxed{
  \ell(w,\pi,T)
...
Notemos que:
$$
\frac{\partial \ell }{\partial w} = -\frac{\psi(\pi-T)}{(1+\psi)w^2} < 0, 
\quad \frac{\partial \ell }{\partial \pi} = \frac{\psi}{(1+\psi)w} > 0, \quad \frac{\partial \ell }{\partial T} = -\frac{\psi}{(1+\psi)w} < 0.
$$
lo que muestra que el efecto sustitución domina al efecto ingreso en la oferta de ocio que resulta de estas preferencias, de modo que la oferta laboral tiene pendiente positiva respecto del salario. Además, al incrementar con los beneficios y disminuir con los impuestos se demuestra que el ocio es un bien normal.
...
La empresa elige $N^d$ para maximizar
\[
  \pi=z\sqrt{N^d}-wN^d.
\]
La C.P.O. es:
...
\]
Despejando, obtenemos la función de demanda laboral:
\[
  \boxed{
  N^d(w)=\frac{z^2}{4w^2}
  }.
\]
Con pendiente negativa respecto al salario:
$$
  \frac{dN^{d}}{dw}=-\frac{z^2}{2w^3}<0.
$$

La oferta de bienes se obtiene sustituyendo la demanda de trabajo en la función de producción:
\[
  Y^s(w)
